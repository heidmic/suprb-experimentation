...
from suprb.rule.matching import MinPercentage
...
def rule_discovery_space(trial: Trial, params: Bunch):
    ...
    params.rule_discovery__mutation__sigma = np.array(
        [trial.suggest_float("sigma_mutate_low", *sigma_space), trial.suggest_float("sigma_mutate_prop", *sigma_space)]
    )
    ...
    if isinstance(params.rule_discovery__init, rule.initialization.NormalInit):
        ...
        params.rule_discovery__init__sigma = np.array(
            [trial.suggest_float("sigma_init_low", *sigma_space), trial.suggest_float("sigma_init_prop", *sigma_space)]
        )
